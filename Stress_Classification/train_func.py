...
def train_PPGECG_model(model, G_AB, train_dataloader, val_dataloader, z_score_norm, min_max_norm, target_fs,
                    num_epochs, optimizer, base_lr, monitor, model_save_path, multi_gpu_flag, DEVICE):
    ...
    def normalize_data(data, z_score_norm=True, min_max_norm=False):
        ...
        for i in range(len(data)):
            ...
            if z_score_norm:
                target_data = (target_data - target_data.mean()) / (target_data.std() + 1e-17)
            if min_max_norm:
                target_data = skp.minmax_scale(target_data, (0, 1), axis=1)
            norm_data.append(target_data)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for iter, train_data in train_t:
            ...
            ppg = batch_signal_downsample(ppg, 128, target_fs)
            ecg = batch_signal_downsample(ecg, 128, target_fs)
...
def train_SYNECG_model(model, G_AB, train_dataloader, val_dataloader, z_score_norm, min_max_norm, is_aug, target_fs,
                    num_epochs, optimizer, base_lr, monitor, model_save_path, multi_gpu_flag, DEVICE):
    ...
    def transform(ecg, transform_list):
        for i, transform in enumerate(transform_list):
            ecg = transform((ecg, 0))[0]

        return ecg

    def augment_ecg(ecg, transform_list):
        ecg = ecg.data.cpu()
        num_of_apply = np.random.choice(len(transform_list))
        apply_sequence = np.arange(0, num_of_apply)
        np.random.shuffle(apply_sequence)

        select_transforms = [transform_list[apply_sequence[i]] for i in range(len(apply_sequence))]

        if len(select_transforms) == 0:
            return ecg.type(torch.FloatTensor)
        else:
            transformed_ecg = []
            for i in range(len(ecg)):
                transformed_ecg.append(transform(ecg[i], select_transforms).data.cpu().numpy())
            return torch.from_numpy(np.array(transformed_ecg)).type(torch.FloatTensor)
    ...
    transform_list = [aug_tools.TTimeOut(crop_ratio_range=[0.0, 0.5]), 
                      aug_tools.TGaussianNoise(scale=0.01),
                      aug_tools.TGaussianBlur1d(),
                      aug_tools.TChannelResize(magnitude_range=(0.33, 3)),
                      aug_tools.TDynamicTimeWarp(warps=3, radius=10, step=2)]
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for iter, train_data in train_t:
            ...
            if is_aug:
                ecg = augment_ecg(ecg, transform_list)
